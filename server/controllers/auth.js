  ...
  try {
    ...
    if (user.status == "Disabled") {
      throw new CustomError(
        "Your account has been diabled! Please contact administrator!",
        403
      );
    }
    let match = await bcrypt.compare(password, user.password);
...
exports.employeeMe = async (req, res, next) => {
  try {
    let employee = await Employee.findById(req.id)
      .populate("salaries")
...
exports.changePasswordEmployee = async (req, res, next) => {
  try {
    if (req.body.token) {
      ...
      res.json({
        message:
          "Password reset successfully! You will be redirected to homepage soon",
        flag: true,
      });
    } else {
