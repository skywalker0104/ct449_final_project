<template>
  <div class="mt-3 pa-2">
    <v-snackbar v-model="snackbar" :timeout="4000" :color="snackbarColor" top>
      {{ message }}
      <template v-slot:action="{ attrs }">
        <v-btn fab color="white" x-small v-bind="attrs" @click="snackbar = false">
          <v-icon color="error" size=23>
            mdi-close-box
          </v-icon>
        </v-btn>
      </template>
    ...
    <v-row justify='end'>
      <v-col>
        <h2 class="textheadpurple--text"> Nhân viên</h2>
      </v-col>
      <v-col class="mb-2">
      ...
      <v-col class="ml-4 pt-1" lg="3" cols=12>
        <v-text-field label="Tìm kiếm" outlined dense class="mt-1" v-model="search"> </v-text-field>
      </v-col>
    </v-row>
        ...
        <v-card max-width="320">
          <div class="text-center">
            <v-col class="pa-4 pt-8">
              ...
              <v-row class="mt-0" justify="center" align="center"></v-row>
