    ...
    <v-navigation-drawer v-model="drawer" app>
      <v-list class="mt-2" nav>
        ...
        <v-list-item v-for="(item, i) in navItems" :key="i" class="pl-8 mt-2 tile" :to="item.route"
          active-class="active--nav" color="white" nuxt @click="$store.commit('head/set', item.title)">
          <v-list-item-icon>
      ...
      <template v-slot:append>
        <div v-if="$auth.loggedIn" class="text-center pb-6">
          <v-btn @click="logoutFn" depressed class="error--text">
...
export default {
  data() {
    return {
      ...
      drawer: true,
      navItems: [
        { title: 'Tổng quan', icon: 'mdi-chart-donut', route: '/' },
        { title: 'Lương', icon: 'mdi-wallet-outline', route: '/salary' },
        { title: 'Dự án', icon: 'mdi-briefcase-outline', route: '/work' },
        { title: 'Thông tin cá nhân', icon: 'mdi-account-outline', route: '/profile' }
      ]
    }
  },
  methods: {
